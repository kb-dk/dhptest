...
D="${1}"
OUTPUT="${2:-output}"
TIFFDIR="${3:-/sbftp-home/scan-dk/DR}"
(
...
echo .
find "${D}" -type d | egrep '\-(P|U)\-' | while read I; do T=0; rm -f numtjek; touch numtjek; find "$I" -type f | egrep -i "xml$" | rev | cut -d'_' -f1 | rev | cut -c1-3 | sort -n | egrep -o '[0-9]*' | while read J; do T=$(echo "$T"+1 | bc); T1=`printf %03d "$T"`; if [ ! "$J" -eq "$T1" ]; then echo ERROR: filename: "$J" - expected: "$T1" >> numtjek; fi; done; TE=$(cat numtjek | wc -l | awk '{print $1}'); if [ "$TE" -ge 1 ]; then echo ERROR in running number for "$I"; cat numtjek; fi; done
echo .
echo checking valid dates
echo .
find "${D}" | grep "xml$" | while read I; do rm -f datotjek; touch datotjek; cat "$I" | grep dateAvailable | egrep -v '[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}' | egrep [0-9] > datotjek; T2=$(cat datotjek | wc -l); if [ "$T2" -ge 1 ]; then echo "$I"; cat datotjek;fi; done
echo .
...
echo .
find "${D}" | grep "xml$" | while read I; do echo $(cat "$I" | grep -v -i identifier | md5sum) $(echo "$I" | rev | cut -d'/' -f1 | rev); done > md5sums; cat md5sums | cut -d' ' -f1 | sort | uniq -c | awk '{print $1" "$2}' | grep -v '^1 ' | cut -d' ' -f2 | while read I; do echo $(grep "$I" md5sums | cut -d' ' -f3); done
echo .
